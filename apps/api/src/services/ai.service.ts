...
const getModel = (client: GoogleGenerativeAI) => {
  return client.getGenerativeModel({ model: process.env.GEMINI_MODEL || 'gemini-1.5-flash' });
};

export const aiService = {
  async analyzeAndVerifyCampaign(campaignId: string): Promise<any> {
    const campaign = await prisma.campaign.findUnique({ where: { id: campaignId } });
    if (!campaign) throw new Error('Campaign not found');

    logger.info(`AI analyzing campaign: ${campaign.name}`);
...
`;
    ...
    logger.info(`AI analysis complete for campaign ${campaignId}: ${verificationStatus}, risk: ${riskScore}`);
    return analysis;
  },

  async generateCampaignSummary(campaignId: string): Promise<string> {
    const campaign = await prisma.campaign.findUnique({ where: { id: campaignId } });
    if (!campaign) throw new Error('Campaign not found');
  ...
  async analyzeImage(imageUrl: string, context: string): Promise<string> {
    try {
      ...
      const model = getModel(client);
      const result = await model.generateContent(
        `Analyze this disaster image URL for authenticity. Context: ${context}. Image URL: ${imageUrl}. Describe what type of disaster evidence would be expected and whether the description matches. Provide a brief analysis.`
      );
      return result.response.text();
    } catch (err) {
